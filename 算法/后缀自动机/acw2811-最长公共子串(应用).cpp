...
void solve() {
    ...
    for(int i = 2; i <= sam.tot; i++) {
        g[sam.nodes[i].fa].push_back(i);
        ans[i] = sam.nodes[i].maxL;
    }
    ...
    for(int i = 2; i <= n; i++) {
        ...
        for(int j = 0; j < t.size(); j++) {
            int c = t[j] - 'a';
            //u 里面所有代表的子串都接不上c, 一直找到一个能接到的为止
            while(u > 1 && sam.nodes[u].ch[c] == 0) u = sam.nodes[u].fa, len = sam.nodes[u].maxL;
            if(sam.nodes[u].ch[c]) {
                u = sam.nodes[u].ch[c];
                len++;
            }
        ...
        /*
        要将数据网上传递。考虑出现了dba, cba 和ba
        如果不往上传，ba 可能miss 掉
        */
        dfs(1);
        for(int i = 2; i <= sam.tot; i++) ans[i] = min(ans[i], dp[i]);
    }

    int res = 0;
    for(int i = 2; i <= sam.tot; i++) res = max(res, ans[i]);
    cout << res << endl;
}
